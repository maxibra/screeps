...
var creep_helpers = {
    create_creep: function() {
        ...
        if (harvesters.length == 0 && global_vars.spawn.memory.general.status != 'peace') { // It's no harversters create a minimum body
    ...
    most_creep_action_results: function(creep, target, action_res, creep_role) {
        switch(action_res) {
            ...
            default:
                ...
                if (creep.memory.role == 'transfer' && creep.memory.target_id != global_vars.spawn.id && global_vars.spawn.energy < global_vars.spawn.energyCapacity) creep.memory.target_id = global_vars.spawn.id;
                else {
