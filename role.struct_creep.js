...
var structCreep = {
    run: function(creep, units) {
        ...
        if ((creep.name.substring(0,7) === 'max_new') && (room_name !== 'E38N47')) {
        ...
        // } else if ((room_vars.status === 'peace' && units[room_name]['upgrade'] < 1 && units[room_name]['total'] >= 3 && creep.ticksToLive > 900) || creep.pos.getRangeTo(Game.rooms[room_name].controller) < 4) {
                    ...
                    units[room_name]['upgrade'] < 1)) && (creep.pos.getRangeTo(far_source) > 5)){
    ...
    // Return here repair
                ...
                } else if (room_name !== 'E39N49' && creep.pos.getRangeTo(far_source) > 5) {
                    creep.say('upgrading');
                    creep.memory.role = 'upgrade';
                }
                // }
        ...
        switch(creep_role) {
            ...
            case 'repair_civilian':
                ...
                console.log('[ERROR] (structCreep): CIVIL ' + creep.name + '; ROLE: ' + JSON.stringify(target));
                    creep_helpers.most_creep_action_results(creep, target, creep.repair(target), creep_role);
                } else {
                    creep.memory.role = 'build';
                    // creep.memory.role = 'undefined';
            ...
            case 'claimer':
                ...
                creep.moveTo(new RoomPosition(18,10,'E38N47'), global_vars.moveTo_ops);   // go to source in new room
                // if (creep.room.name === 'E34N46' || creep.room.name === 'E34N47') creep.moveTo(new RoomPosition(43,20,'E34N47'), global_vars.moveTo_ops);
                if (creep.room.name === 'E38N47') creep.memory.role = 'undefined';
                break;
            case 'attack':
