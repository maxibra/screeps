...
RoleTower = {
    run: function (current_tower_id, creeps_amount) {
        ...
        console.log('[INFO] (RoleTower.run) Room status: ' + Game.spawns[spawn_name].memory.general.status);
        if (Game.spawns[spawn_name].memory.general.status == 'war') {
            ...
            target2repair = Game.getObjectById(Game.rooms[room_name].memory.target_repair_defence);
...
//        console.log('[DEBUG] (RoleTower.run)[ ' + current_tower.id + ']: To Attack: ' + (target2attack?target2attack.length:0) + '; To repair: ' + (target2repair?target2repair.length:0));
...
//        console.log('[DEBUG] (RoleTower.run)[' + current_tower_id + ']: Targets to attack: ' + target2attack.length);
        if (target2attack && target2attack.length > 0) {
            console.log('[DEBUG] (RoleTower.run)[' + current_tower_id + ']: Attack (' + target2attack.id)
